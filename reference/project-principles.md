...
2. **Frontal symbology convention:** semicircles = warm front, triangles = cold front. Symbols are drawn **on the frontal line** (never floating beside it) and point **in the direction of frontal movement**. Used identically in every diagram that shows a front — cross-sections and chart views alike.
   **Orientation rule for cross-sections:** site convention is that frontal cross-sections advance **left to right, into the cold/retreating air drawn on the right** — every front diagram shares this orientation so cold vs. warm fronts contrast only in slope and weather character. A front always advances toward the air mass it is replacing, so the movement arrow must point into the retreating air. When the spatial order of weather features runs opposite the reading direction, carry the observer's encounter order with numbered stations (① = seen first).
